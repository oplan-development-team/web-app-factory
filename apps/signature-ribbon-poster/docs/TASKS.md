...
- [ ] **T03** `core/poster.ts` / `core/geometry.ts`: ポスター空間定数と clamp/lerp/distance/midpoint — FR-001.8
...
- [ ] **T10** `core/draft.ts`: ドラフトの直列化・復元・スキーマ検証 — FR-011.3, FR-011.4
- [ ] **T11** `core/draft-storage.ts`: localStorage リポジトリ（例外を投げない・破損時削除） — FR-011.4, FR-011.5, E-08, E-09

## Phase C — render（描画層）

- [ ] **T12** `render/types.ts`: `Ctx2D` / `CanvasLike` / `CanvasFactory` と、テスト用フェイク ctx — NFR-003.3
- [ ] **T13** `render/ribbon-painter.ts`: コア層への描画（中点二次ベジェ・可変幅・ホットコア・単点ドット） — FR-004.1, FR-004.3, E-01
...
- [ ] **T19** `app/studio.ts`: 状態の単一の持ち主（ストローク・履歴・設定・変更通知） — FR-005.2, FR-006.2, FR-008, FR-013.3
- [ ] **T20** `app/pointer-input.ts`: Pointer Events の処理（キャプチャ・単一ポインタ・確定） — FR-001.1〜FR-001.4, E-03, E-04
- [ ] **T21** `app/exporter.ts`: 書き出しパイプライン（オフスクリーン合成 → PNG → ダウンロード） — FR-009, FR-010.3, E-13
- [ ] **T22** `app/draft-sync.ts`: 自動保存（800ms デバウンス）と復元の適用 — FR-011.1, FR-011.6
...
- [ ] **T31** E2E（Playwright）: 描画 / 色変更 / Undo・Redo / キャプション / 書き出し / 復元 / リサイズ — NFR-002.4
- [ ] **T32** ベンチマーク（`bench/`）: 新旧比較の実測と README への記載 — NFR-001.5, NFR-001.8
