...
- [x] **T31** `README.md` を本実装の内容へ更新（生成モデルの説明を差し替え） — 依頼事項 #6
- [x] **T32** `docker build` の疎通確認と、ビルド・目視・コンタクトシートによる最終検証 — AC-27, AC-29
...
| 項目 | 結果 |
|---|---|
| `npm run build` | 型エラー 0。JS 30.06KB（gzip 10.89KB / 予算 40KB）、CSS 12.85KB（gzip 3.47KB） |
| `npm test` | 174 件すべて成功。行 97.1% / 分岐 86.3% / 関数 97.4%（しきい値 80%） |
| `npm run test:e2e` | 62 件成功・1 件スキップ（WebKit の Tab 送りは既定挙動のため対象外） |
| `docker build` → 起動 | イメージ生成成功。コンテナ起動後 `/` が 200。ブラウザ操作一巡でコンソール ERROR 0 件 |
| SVG 書き出し | 単体で開いて 400×400 で描画。外部参照・`<script>`・`<image>` を含まない |
| PNG 書き出し | IHDR が 1200×1200。地色が塗られている（透過でない） |
| 横スクロール | 320 / 375 / 768 / 1024 / 1440px で 0（3 ブラウザ） |

検証後、確認に使ったコンテナとイメージは削除済み。

### 実装中に見つけて直した不具合

- `hidden` 属性が `display` を持つクラスに負け、確認 UI が常時見えていた
- `input` の固有幅（size=20 相当・実測 346px）とグリッド項目の `min-width: auto` により、
  Firefox / WebKit の 320px 幅で 76px はみ出していた（Chromium では再現しない）
