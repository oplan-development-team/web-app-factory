...
| 紙 | 多層グラデーション + SVGノイズの全面固定オーバーレイ + ヴィネット |
| インク | 墨 `#1c1a17` |
| アクセント | 朱色 `#b8371f`。EDITOR'S PICK / ランキング1〜3位 / 現在地 / 必須・エラーのみに使う |
| 罫線 | 1px罫と3px二重罫。`border-radius` は原則 0 |
...
外部画像をランタイムに取得しません。カテゴリー別の 8 モチーフ（窓辺・ノート・机上・皿・
山道・棚と鉢・本棚・抽象）を、記事IDのハッシュから決定的に揺らして **インラインSVGで描画**
しています。全体を単一のセピアインクランプで塗り、網点スクリーンを重ねて新聞写真に寄せています。

同じ記事は常に同じ絵になります（描画経路に `Math.random()` を使っていません）。
